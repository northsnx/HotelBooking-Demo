...
const Chatbot = ({ isOpen, onClose, searchParams, currentRoom }) => {
  const [messages, setMessages] = useState([
    { type: 'bot', text: "สวัสดีครับ 😊 ผมเป็นผู้ช่วยของคุณนะครับ พร้อมช่วยแนะนำห้องพัก ตรวจสอบราคา หรือจองห้องที่คุณสนใจอยู่ หากต้องการจอง สามารถพิมพ์คำว่า 'ยืนยัน' ได้เลยครับ 🌟" }
  ]);
  ...
  if (text.includes("สวัสดี") || text.includes("hello") || text.includes("hi")) {
    return "สวัสดีครับ 😊 ต้องการให้ผมช่วยค้นหาห้องพัก แนะนำราคา หรือสรุปการจองไหมครับ?";
  }

  // ------------------------------------
  // 1. การยืนยันการจอง (สร้างบิล)
  // ------------------------------------
  if (
  ...
  // ------------------------------------
  // 2. ถามราคา หรือ ราคาห้องประเภทต่างๆ
  // ------------------------------------
  if (text.includes("ราคา") || text.includes("เท่าไร") || text.includes("กี่บาท")) {
    const priceList = roomsData
      .map((r) => `• ${r.name}: ${r.price.toLocaleString()} บาท/คืน`)
      .join("\n");
    return `นี่คือราคาห้องพักทั้งหมดของเรา:\n${priceList}\n\nหากต้องการดูรายละเอียดห้องไหน พิมพ์ชื่อห้อง เช่น "ห้องเดี่ยว" ได้เลยครับ`;
  }

  // ------------------------------------
  // 3. ถามจำนวนคนที่รองรับ
  // ------------------------------------
  if (text.includes("รับกี่คน") || text.includes("กี่คน") || text.includes("ความจุ")) {
    return roomsData
      .map(
        (r) =>
          `• ${r.name} รองรับผู้ใหญ่ ${r.max_adults} คน + เด็ก ${r.max_children} คน`
      )
      .join("\n");
  }
  ...
  // 4. ถามเรื่องมัดจำ
  // ------------------------------------
  if (text.includes("มัดจำ") || text.includes("เงินประกัน")) {
    const depositInfo = roomsData
      .filter((r) => r.deposit > 0)
      .map((r) => `• ${r.name}: มัดจำ ${r.deposit.toLocaleString()} บาท`)
      .join("\n");
    return `ข้อมูลค่ามัดจำของแต่ละห้อง:\n${depositInfo}`;
  }

  // ------------------------------------
  // 5. ถามชื่อห้องที่เฉพาะเจาะจง เช่น “ห้องเดี่ยว”
  // ------------------------------------
  const foundRoom = roomsData.find((r) => text.includes(r.name.split(" ")[0].toLowerCase()));
  if (foundRoom) {
    return (
      `ℹ️ รายละเอียดห้อง **${foundRoom.name}**\n` +
      `• ราคา: ${foundRoom.price.toLocaleString()} บาท/คืน\n` +
      (foundRoom.deposit ? `• มัดจำ: ${foundRoom.deposit.toLocaleString()} บาท\n` : "") +
      `• รองรับ: ผู้ใหญ่ ${foundRoom.max_adults} + เด็ก ${foundRoom.max_children}\n` +
      `• รายละเอียด: ${foundRoom.description}\n\n` +
      `หากต้องการจองพิมพ์ "ยืนยัน" ได้เลยครับ`
    );
  }

  // ------------------------------------
  // 6. ถามเวลาเช็คอิน/เอาท์
  // ------------------------------------
  if (text.includes("เช็คอิน") || text.includes("check in"))
    return `เวลาเช็คอินคือ ${hotelInfo.policies.checkIn} ครับ`;
  if (text.includes("เช็คเอาท์") || text.includes("check out"))
    return `เวลาเช็คเอาท์คือ ${hotelInfo.policies.checkOut} ครับ`;

  // ------------------------------------
  // 7. ถามอาหารเช้า
  // ------------------------------------
  if (text.includes("อาหารเช้า") || text.includes("breakfast"))
    return hotelInfo.policies.breakfast;

  // ------------------------------------
  // 8. ถามที่ตั้งโรงแรม
  // ------------------------------------
  if (text.includes("ที่ไหน") || text.includes("อยู่ไหน") || text.includes("location"))
    return `โรงแรมของเราตั้งอยู่ที่: ${hotelInfo.location} ครับ`;

  // ------------------------------------
  // 9. ถามสิ่งอำนวยความสะดวก
  // ------------------------------------
  if (
    text.includes("มีอะไรบ้าง") ||
    text.includes("facilities") ||
    text.includes("สิ่งอำนวยความสะดวก")
  ) {
    return `สิ่งอำนวยความสะดวกของเรามี: ${hotelInfo.facilities.join(", ")} ครับ`;
  }

  // ถามเฉพาะบางสิ่ง เช่น ฟิตเนส สระว่ายน้ำ
  if (text.includes("ฟิตเนส")) return "มีฟิตเนสเปิด 06:00–22:00 น. ครับ";
  if (text.includes("สระ") || text.includes("สระว่ายน้ำ"))
    return "สระว่ายน้ำเปิดบริการ 08:00–20:00 น. ครับ";

  // ------------------------------------
  // 10. นโยบายยกเลิก
  // ------------------------------------
  if (text.includes("ยกเลิก") || text.includes("cancel"))
    return hotelInfo.policies.cancellation;

  // ------------------------------------
  // 11. คำถามเกี่ยวกับ WiFi
  // ------------------------------------
  if (text.includes("wifi") || text.includes("ไวไฟ"))
    return hotelInfo.policies.wifi;

  // ------------------------------------
  // 12. หากไม่เข้าใจคำถาม
  // ------------------------------------
  return (
    "ผมยังไม่เข้าใจคำถามครับ 😅\n" +
    "คุณสามารถถามผมเรื่องต่อไปนี้ได้:\n" +
    "• ราคาแต่ละห้อง\n• ข้อมูลมัดจำ\n• เช็คอิน / เช็คเอาท์\n• สิ่งอำนวยความสะดวก\n• ที่ตั้งโรงแรม\n• รายละเอียดห้องเฉพาะชื่อ\n• หรือพิมพ์ “ยืนยัน” เพื่อจองห้อง"
  );
};
